...
all:		main.c
			$(CC) $(CFLAGS) -o $(PROG) main.c $(RESISTANCE_LIBFLAG) $(COMPONENT_LIBFLAG) # Add rest of libs here

install:	
			sudo rm -f *~ *.o $(PROG) # just to be sure that $(PROG) is rebuilt later. Is this necessary?
	 		sudo install -m 0755 $(LIBRESISTANCE) $(LIBCOMPONENT) $(LOCALLIBDIR) # copies the libraries. Add your libs here
	 		$(CC) $(CFLAGS) -o $(PROG) main.c $(RESISTANCE_LIBFLAG_PUBLIC) $(COMPONENT_LIBFLAG_PUBLIC) # add your libs here
